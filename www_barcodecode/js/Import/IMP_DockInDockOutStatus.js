...
function GetVCTDetails() {
    ...
    if (txtVCTNo.length >= '8') {
        if (errmsg == "" && connectionStatus == "online") {
            $.ajax({
                type: 'POST',
                url: CMSserviceURL + "GetVCTDetails",
                data: JSON.stringify({
                ...
                success: function (response) {
                    ...
                    if (response != null && response != "") {
                        ...
                        $(xmlDoc).find('Table').each(function (index) {
                            ...
                            if (DockInStatus == 'false' && OutMsg != 'InValid Token No.') {
...
function UpdateVCTStatus(D_Status) {
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: CMSserviceURL + "UpdateVCTStatus",
            data: JSON.stringify({
                'pi_strVCTNo': txtVCTNo,
                //'pi_VCTDatetime': newDate,
                'pi_strUserId': UserName,
...
function openScannerForVTNoScan() {
