...
function UnitizationBuiltUpPauseDetail(val) {
    ...
    var _seqqNo;
    if ($("#bulkCheckBox").prop("checked")) {
        _seqqNo = $("#trolleyLists").val();
    } else {
        _seqqNo = $("#uldLists").val();
    }

    var InputXML = "<Root><FLTSEQNO>" + FltSeqNo + "</FLTSEQNO><ULDSEQNO>" + _seqqNo + "</ULDSEQNO><REASON>" + $('#ddlReason').val() + "</REASON><CREATEDBY>" + Userid + "</CREATEDBY><AIRPORT_CITY>" + SHED_AIRPORT_CITY + "</AIRPORT_CITY><ACTIONTYPE>" + val + "</ACTIONTYPE><IsTrolly>" + setIsTrolley + "</IsTrolly></Root>";
    ...
    $.ajax({
        type: 'POST',
        url: ExpURL + "/UnitizationBuiltUpPauseDetail",
        data: JSON.stringify({ 'InputXML': InputXML }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response, xhr, textStatus) {
